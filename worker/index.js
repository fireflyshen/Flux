...
async function handleApi(request, env, url) {
  if (request.method !== "GET")
    return json({ error: "Flux 只提供只读账本接口" }, 405);
  ...
  if (url.pathname === "/api/days" || url.pathname === "/api/reports") {
    ...
    if (!Number.isInteger(year) || year < 1900 || year > 2200)
      return json({ error: "年份无效" }, 400, version);
    return json(
      snapshot.years?.[String(year)] ?? emptyYear(snapshot, year),
      200,
      version,
    );
  }

  const date = url.pathname.startsWith("/api/days/")
    ? url.pathname.slice("/api/days/".length)
    : "";
  if (date) {
    if (!validDate(date)) return json({ error: "日期无效" }, 400, version);
    const yearData =
      snapshot.years?.[date.slice(0, 4)] ??
      emptyYear(snapshot, Number(date.slice(0, 4)));
    return json(
      yearData.days.find((day) => day.date === date) ?? {
        date,
        totals: {},
        categories: [],
        transactions: [],
  ...
  if (url.pathname === "/api/health")
    return json(
      { ok: true, generatedAt: snapshot.generatedAt, version },
      200,
      version,
    );

...
function comparisonCopy(amount: number, median: number) {
  ...
  if (amount < median * 0.88) return `比当年典型支出低 ${difference}%`;
  return "接近当年的典型日常支出";
}
...
export function DayDrawer({
  ...
  const transactions = useMemo(
    () =>
      day?.transactions.flatMap((transaction) => {
        ...
        return total.gross === 0 && total.refunds === 0 && total.net === 0
          ? []
          : [{ transaction, total }];
      }) ?? [],
  ...
  const categories = useMemo(() => {
    ...
    for (const { transaction } of transactions) {
      for (const category of transaction.categories.filter(
        (item) =>
          item.currency === currency && (!account || item.account === account),
      )) {
        const current = rows.get(category.account);
        if (!current) rows.set(category.account, { ...category });
        else {
          current.gross = String(
            Number(current.gross) + Number(category.gross),
          );
          current.refunds = String(
            Number(current.refunds) + Number(category.refunds),
          );
  ...
  const maxCategory = Math.max(
    0,
    ...categories.map((item) => Number(item.gross)),
  );
        ...
        >
          <div className="page-heading spend-heading">
            ...
            <div className="page-meta">
              <strong>
                {detailAvailable
                  ? comparisonCopy(gross, median)
                  : "这一天缺少交易明细"}
              </strong>
              {detailAvailable && (
                <span>
                  退款 {formatMoney(refunds, currency)} · 净支出{" "}
          ...
          <nav className="mobile-section-switch" aria-label="日期内容">
            <button
              type="button"
              data-active={mobileSection === "categories"}
              onClick={() => showMobileSection("categories")}
            >
              分类 <span>{categories.length}</span>
            </button>
            <button
              type="button"
              data-active={mobileSection === "transactions"}
              onClick={() => showMobileSection("transactions")}
            >
              交易 <span>{transactions.length}</span>
            </button>
          </nav>

          <section
            className="spend-category-section"
            aria-labelledby="category-title"
          >
            <div className="drawer-section-heading">
              <h3 id="category-title">行为支出分类</h3>
            ...
            {categories.length > 0 ? (
              <div className="category-list">
                {categories.map((category) => {
                  ...
                  return (
                    <article className="category-row" key={category.account}>
                      <div>
                      ...
                      <div className="report-bar" aria-hidden="true">
                        <i
                          style={{
                            width: `${maxCategory <= 0 ? 0 : (Math.max(0, value) / maxCategory) * 100}%`,
                          }}
                        />
                      </div>
                    </article>
              ...
              <p className="drawer-empty">这一天没有 {currency} 支出分类</p>
            )}
          </section>
          ...
          >
            <div className="drawer-section-heading">
              <h3 id="transaction-title">交易明细</h3>
              <span>{transactions.length} 笔</span>
            </div>
            {transactions.length > 0 ? (
              <div className="transaction-list">
                {transactions.map(({ transaction, total }) => {
                  return (
              ...
              <p className="drawer-empty">这一天没有 {currency} 支出交易</p>
            )}
          </section>
        </div>
      ) : (

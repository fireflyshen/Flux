...
export function Heatmap({
  ...
  const thresholds = useMemo(
    () =>
      quantileThresholds(
        ...
        currency,
        account
          ? (day, selectedCurrency) =>
              accountBehaviorAmountOf(day, account, selectedCurrency)
          : behaviorGrossAmountOf,
      ),
    [account, currency, days],
  );
  const monthLabels = useMemo(() => {
    const labels: { month: string; week: number }[] = [];
    let lastMonth = -1;
    weeks.forEach((week, index) => {
      const visibleDay = week.find(
        (day) => day.inYear && fromDateKey(day.date).getMonth() !== lastMonth,
      );
      if (visibleDay) {
  ...
  useEffect(() => {
    ...
    const frame = window.requestAnimationFrame(() => {
      ...
      container.scrollLeft = Math.max(
        0,
        targetLeft - container.clientWidth + 72,
      );
    });
  ...
  return (
    <div
      ...
      tabIndex={0}
      aria-label={`${year} 年 ${currency} ${accountName}支出热力图`}
    >
        ...
        <div className="heatmap-grid">
          {weeks.map((week) => (
            <div className="heatmap-week" key={week[0].date}>
              {week.map((day) => {
                if (!day.inYear)
                  return <span className="heatmap-blank" key={day.date} />;
                const amount = account
                  ? accountBehaviorAmountOf(day.summary, account, currency)
                  : behaviorGrossAmountOf(day.summary, currency);
                const refunds = account
                  ? accountBehaviorAmountOf(
                      day.summary,
                      account,
                      currency,
                  ...
                  : behaviorAmountOf(day.summary, currency, "refunds");
                const detailAvailable = behaviorDetailAvailable(
                  day.summary,
                  currency,
                );
                ...
                const dateLabel = new Intl.DateTimeFormat("zh-CN", {
                  ...
                  day: "numeric",
                  weekday: "short",
                }).format(fromDateKey(day.date));
                const statusLabel = !day.summary
                  ? "没有支出记录"
                  : !detailAvailable
                    ? "缺少交易明细，无法计算行为支出"
                    : `${account ? accountName : "行为"}毛支出 ${formatMoney(amount, currency)}${refunds > 0 ? `，退款 ${formatMoney(refunds, currency)}` : ""}`;
                return (
                  <button
                    type="button"
